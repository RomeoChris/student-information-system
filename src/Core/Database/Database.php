...
class Database implements IDatabase
{
	...
	{
		return $this->query("DELETE FROM $table WHERE $where", $data);
	}

	public function getWhere(string $table, string $where, array $data) :array
	{
		$fetchedData = $this->query("SELECT * FROM $table WHERE $where", $data, self::ROW);
		return $fetchedData ? $fetchedData : [];
	}
	...
	public function selectAll(string $table) :array
	{
		...
		return $this->query($queryString, [], self::ALL);
	}
